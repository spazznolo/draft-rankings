...
draft_rankings <- 
  ...
  filter(rank <= 30, ranking_size > 30) %>%  # Keep only the top 100 ranks
...
R <- as.rankings(part_ranking_matrix)

# Standard maximum likelihood estimates
mod_mle <- PlackettLuce(R, npseudo = 0.5, weights = (1:65)/65)

mle_estimates <- coef(mod_mle, log = FALSE)
...
draft_simulations <- 
  replicate(100000, sample(1:80, 80, replace = FALSE, prob = mle_estimates)) %>%  # Simulate 100,000 draft rankings based on the estimated probabilities
  matrix(., nrow = 80, ncol = 100000)  # Convert the simulated rankings to a matrix format

list_sims <- map(1:30, ~rowSums(draft_simulations == .) / 100000)  # Calculate the probabilities for each rank for each skater
...
draft_probabilities <-
  ...
  rename_at(2:ncol(.), ~paste0('rank_', parse_number(.)))  # Rename the columns to represent the ranks
